...
import { RefreshCw, Zap, DollarSign, BarChart3, TrendingUp, AlertCircle } from "lucide-react";
...
export default function CostPage() {
  ...
  const maxCost = data
    ? Math.max(data.cost_aks_cpu_per_1k_requests_usd, data.cost_aks_gpu_per_1k_requests_usd, 0.01)
    : 0.01;
    ...
    <div className="p-6 max-w-4xl">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">LLMOps token usage and deployment cost projections</p>
        </div>
        <button
          onClick={fetch}
          disabled={loading}
      ...
      {data && (
        <Card className="mb-6">
          <CardHeader
            ...
            subtitle="Cost per 1,000 requests across infrastructure options"
          />
          <div className="p-5 space-y-4">
            <Bar
              label="Local (RTX 5080) — current"
              value={0}
              max={maxCost}
              color="bg-emerald-500"
            />
            <Bar
              label="AKS CPU (D4s_v3, $0.192/hr)"
              value={data.cost_aks_cpu_per_1k_requests_usd}
              max={maxCost}
              color="bg-yellow-500"
            />
            <Bar
              label="AKS GPU (NC6s_v3 + vLLM, $3.06/hr)"
              value={data.cost_aks_gpu_per_1k_requests_usd}
              max={maxCost}
              color="bg-orange-500"
            />
          ...
          <div className="p-4 grid md:grid-cols-3 gap-4">
            {Object.entries(breakdown).map(([key, val]) => (
              <div key={key} className="bg-[#1d2335] rounded-xl p-4 space-y-2">
                <p className="text-indigo-300 text-xs font-mono uppercase tracking-wider">{key}</p>
